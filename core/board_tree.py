...
class BoardTree(object):
    def __init__(self, treeFile, lookupFile=None):
        ...
        if isinstance(treeFile, dict):
            ...
            self.root = load_binary_zip_Json(treeFile)
        if lookupFile is None:
            ...
            self.manualLookup = False
            self._lookup = load_binary_zip_Json(lookupFile)
    ...
    def addNodes(self, g, rootName, seq, depth=None, evalFactor=1):
        ...
        for k, v in list(node.items()):
            ...
            if len(v) > 2:
                g = self.addNodes(g, f"{rootName},{k}", seq + [k], depth=depth - 1 if depth else None,
                                  evalFactor=evalFactor)
        return g

    def drawSeqTree(self, seq, ax=None, depth=4, rootName='root', minHits=None, showLabels=True, labelDepth=2,
                    scalingFactor=.01):
        node = self[seq]
        ...
        try:
            ...
            pos = {k: ((p1 - meanX) * scalingFactor * (1 if k not in topn else 1.5), p2 * scalingFactor) for k, (p1, p2)
                   in pos.items()}
        except ImportError:
        ...
        nx.draw_networkx(G,
                         ...
                         node_color=[
                             int(d['sfeval'].replace('#', '')) * 10 if isinstance(d['sfeval'], str) else d['sfeval'] for
                             n, d in G.nodes(data=True)],
...
def load_binary_zip_Json(filename):
